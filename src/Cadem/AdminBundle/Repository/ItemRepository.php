...
class ItemRepository extends EntityRepository
{
    public function findAllOrderedByName()
    {
        return $this->getEntityManager()
                    ->createQuery("SELECT i FROM Cadem\AdminBundle\Entity\Item i ORDER BY i.nombre ASC")					
					->setMaxResults(5)
                    ->getResult();					
    }
   ...
   */
   public function ajaxTable(array $get, $flag = false)
   {
    ...
    /* DB table to use */
    $tableObjectName = 'CademAdminBundle:Item';
    ...
    $cb = $this->getEntityManager()
      ...
      ->select('i.id id', 'i.nombre nombre', 'i.codigo codigo', 'm.nombre marca', 'f.nombre fabricante', 'i.activo activo')
	  ->leftJoin('i.marca','m')
	  ->leftJoin('i.fabricante','f');
  ...
  public function getCount(){
    $aResultTotal = $this->getEntityManager()
      ->createQuery('SELECT COUNT(i) FROM CademAdminBundle:Item i')
      ->setMaxResults(1)
      ->getResult();
